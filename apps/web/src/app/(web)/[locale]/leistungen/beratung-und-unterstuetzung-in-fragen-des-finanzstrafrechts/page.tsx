...
import {adviceInFinancialCriminalLawAdvantages} from "@/assets/data/messages/adviceInFinancialCriminalLaw";
...
export async function generateMetadata({params}: { params: { locale: string; } }) {
  ...
  const t = await getTranslations("AdviceInFinancialCriminalLawPage");

  return generatePageMetadata(`/${currentLocale}/beratung-und-unterstuetzung-in-fragen-des-finanzstrafrechts`, t("HeroArticle.title"), t("HeroArticle.description"));
}

export default function Page() {

  const t = useTranslations('AdviceInFinancialCriminalLawPage');
  ...
  return (
    <div>
      ...
      <Article>
        ...
        >
          <Grid maxCols={2}>
            {adviceInFinancialCriminalLawAdvantages.map(item => (
              <Box
                Icon={Check}
        ...
        >
          <Grid>
            {partners.map(item => (
              <Box
                alignItems={"center"}
                description={t(`PartnersArticle.${item.messageKey}.description`)}
                image={{
                  src: item.image,
                  alt: t(`PartnersArticle.${item.messageKey}.title`),
                }}
                key={item.messageKey}
              />
            ))}
