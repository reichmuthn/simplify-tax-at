...
import {
  annualFinancialStatementsAndAccountingAdvantages,
  annualFinancialStatementsAndAccountingFaqs
} from "@/assets/data/messages/annualFinancialStatementsAndAccounting";
...
export async function generateMetadata({params}: { params: { locale: string; } }) {
  ...
  const t = await getTranslations("AnnualFinancialStatementsAndAccountingPage");

  return generatePageMetadata(`/${currentLocale}/jahresabschluss-und-bilanzierung`, t("HeroArticle.title"), t("HeroArticle.description"));
}

export default function Page() {

  const t = useTranslations('AnnualFinancialStatementsAndAccountingPage');
  ...
  return (
    <div>
      ...
      <Article>
        ...
        >
          <Grid maxCols={2}>
            {annualFinancialStatementsAndAccountingAdvantages.map(item => (
              <Box
                Icon={Check}
      ...
      <Article>
        <ArticleLayout
          columns={2}
          subtitle={t("FaqArticle.subtitle")}
          title={t("FaqArticle.title")}
          description={t("FaqArticle.description")}
        >
          <Faq faqs={annualFinancialStatementsAndAccountingFaqs.map(item => (
            {
              question: t(`FaqArticle.${item.messageKey}.question`),
              answer: t.rich(`FaqArticle.${item.messageKey}.answer`, {
                p: (chunks) => <p>{chunks}</p>,
